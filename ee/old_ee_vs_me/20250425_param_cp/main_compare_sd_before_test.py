...
from modules import CrossEntropyLossT, FixedLoss, SchedulerCountingWrapper
import utils
...
train_ds = base_train_ds.transform(train_trans).in_ndata(256)
...
for i in range(1):
    ...
    # for net in [gitnet_def, gitnet, gitnet_bn, gitnet_bn_def]:
        ...
        scheduler_t = (SchedulerCountingWrapper(torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=epochs, eta_min=0, last_epoch=-1)), "epoch")
        ...
        etrainer.scheduler_t = (SchedulerCountingWrapper(torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=epochs, eta_min=0, last_epoch=-1)), "epoch")
        ...
        for e in range(epochs):
            ...
            met_dict = {"epoch": e + 1, "lr": lr, "train_loss": train_loss, "train_acc": train_acc, "val_loss": val_loss, "val_acc": val_acc}
        ...
        for e in range(epochs):
            lr = etrainer.get_lr()
            run_mgr.log_metric("lr", lr, step=e + 1)
