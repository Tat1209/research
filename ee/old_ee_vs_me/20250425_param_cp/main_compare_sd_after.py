...
for i in range(1):
    ...
    # for net in [gitnet_def, gitnet, gitnet_bn, gitnet_bn_def]:
        ...
        run_mgr.log_param("optimizer", optimizer.__class__.__name__)
        ...
        scheduler_t = (torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=epochs, eta_min=0, last_epoch=-1), "epoch")
        merge_ens = MergeEnsemble(me_trainers, loss_func=loss_func, optimizer=optimizer, scheduler_t=scheduler_t, device=device)


        # -----------------------------------------------------------------------        
        network_ee = ee_trainer.network
        networks_me = [me_trainers[i].network for i in range(64)]
        ...
        me_sd = merge_ens.trainers[0].network.state_dict()
        sum = 0
        for k in ee_sd.keys():
            if ee_sd[k].dim() == 0:
                pass
