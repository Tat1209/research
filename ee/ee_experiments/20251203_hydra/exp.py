...
@hydra.main(version_base=None, config_path=".", config_name="config_single")
def exp(cfg: DictConfig):
    src_text, src_name= utils.get_source(path=this_path, with_name=True)

    device = utils.get_device(avoid_used=False, lock_path=work_path / "tmp/gpu_locks", free_mem_th=5000)

    exp_name = cfg.exp_name

    net = getattr(models, cfg.model_name)

    train_ds_str = cfg.dataset.train_ds_str
    val_ds_str = cfg.dataset.val_ds_str

    base_epochs = cfg.base_epochs
    optim_str, max_lr = cfg.optim_lr["optim"], cfg.optim_lr["lr"]
    ...
    run_mgr = RunManager(exc_path=cfg., exp_tpl="exp_tpl_ee")
    # run_mgr = RunManager(exc_path=this_path, exp_tpl="exp_tpl_ee")
