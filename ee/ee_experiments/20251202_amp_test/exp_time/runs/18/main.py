        ...
        for ndata, epochs in ndata_epochs_l:
            for wd in wd_l:
                ...
                for div in div_l:
                    ...
                    network = torch.compile(Network(EERefiner(net(num_classes=num_classes)).cifar_style().multi_narrow(div=div, agg="both").init_weights().build().to(device)))
                    ...
                    trainer = EETrainer(network, criterion, optimizer, scheduler, device, dtype=torch.bfloat16)
                ...
                # runs_mgr.log_torch_save(mtrainer.networks.get_sd(), "state_dict.pt")  # get_sdはいまない
