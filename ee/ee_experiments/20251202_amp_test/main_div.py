    ...
    for optim_str, max_lr in optim_lr:
        for ndata, epochs in ndata_epochs_l:
            for wd in wd_l:
                ...
                network = Network(EERefiner(net(num_classes=num_classes)).cifar_style().multi_narrow(div=div, agg="both").init_weights().build().to(device))
                ...
                hp_dict = {
                    "num_params": trainer.network.param_stat(lambda p: p.numel()),
                ...
                for e in range(epochs):
                    ...
                    # runs_mgr.log_metrics(trainer.time_info(), step=e + 1)
                    runs_mgr.log_metrics(trainer.time_stats(incl_fmt=False), step=e + 1)
                    runs_mgr.log_metrics(trainer.time_stats_mt(incl_fmt=False), step=e + 1)
                ...
                # runs_mgr.log_torch_save(trainer.network.get_sd(), "state_dict.pt")  # get_sdはいまない
