...
def run(cfg: dict):
    ...
    match train_ds_str:
        case "cifar10_train" | "cifar100_train":
            # 32x32: Padding 4px -> Crop 32, Horizontal Flip
        ...
        case "stl10_train":
            # 96x96: Padding 12px -> Crop 96, Horizontal Flip
            train_trans = [transforms.ToImage(), transforms.RandomCrop(96, padding=12), transforms.RandomHorizontalFlip(), transforms.ToDtype(torch.float32, scale=True), base_train_ds.normalizer()]
            val_trans = [transforms.ToImage(), transforms.ToDtype(torch.float32, scale=True), base_train_ds.normalizer()]

        case "tiny_imagenet_train":
            # 64x64: Padding 8px -> Crop 64, Horizontal Flip
            train_trans = [transforms.ToImage(), transforms.RandomCrop(64, padding=8), transforms.RandomHorizontalFlip(), transforms.ToDtype(torch.float32, scale=True), base_train_ds.normalizer()]
            val_trans = [transforms.ToImage(), transforms.ToDtype(torch.float32, scale=True), base_train_ds.normalizer()]

        case "svhn_train":
            # 32x32: Padding 4px -> Crop 32, NO Flip
            train_trans = [transforms.ToImage(), transforms.RandomCrop(32, padding=4), transforms.ToDtype(torch.float32, scale=True), base_train_ds.normalizer()]
            val_trans = [transforms.ToImage(), transforms.ToDtype(torch.float32, scale=True), base_train_ds.normalizer()]

        case "mnist_train":
            # 28x28: Typically No Augmentation for standard baseline
            train_trans = [transforms.ToImage(), transforms.ToDtype(torch.float32, scale=True), base_train_ds.normalizer()]
            val_trans = [transforms.ToImage(), transforms.ToDtype(torch.float32, scale=True), base_train_ds.normalizer()]

        case "fashion_mnist_train":
            # 28x28: Padding 4px -> Crop 28, Horizontal Flip is acceptable for clothing
            train_trans = [transforms.ToImage(), transforms.RandomCrop(28, padding=4), transforms.RandomHorizontalFlip(), transforms.ToDtype(torch.float32, scale=True), base_train_ds.normalizer()]
            val_trans = [transforms.ToImage(), transforms.ToDtype(torch.float32, scale=True), base_train_ds.normalizer()]

        case "caltech101_train" | "cub200_train" | "flower102_train":
            # Variable Sizes: Resize to 256 -> Random Crop 224
            train_trans = [transforms.ToImage(), transforms.Resize(256), transforms.RandomCrop(224), transforms.RandomHorizontalFlip(), transforms.ToDtype(torch.float32, scale=True), base_train_ds.normalizer()]
            # Validation must typically use CenterCrop to match training dimensions
            val_trans = [transforms.ToImage(), transforms.Resize(256), transforms.CenterCrop(224), transforms.ToDtype(torch.float32, scale=True), base_train_ds.normalizer()]
    ...
    exp.ref_results() # 競合しないように，書き込みは一回だけ
