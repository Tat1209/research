...
exp_name = "exp_sync_mod_da_2"
# exp_name = "exp_tmp"

# net = resnet18_ee
net = resnet18_old_cifar_ee
...
for optim_str, max_lr in optim_lr:
    for wd in wd_l:
        match train_ds_str:
            ...
            #     val_trans = [transforms.ToImage(), transforms.ToDtype(torch.float32, scale=True), base_train_ds.normalizer()]
            case "cifar100_train":
                train_trans = [transforms.ToImage(), transforms.RandomCrop(32, padding=4), transforms.RandomHorizontalFlip(), transforms.ToDtype(torch.float32, scale=True), transforms.Normalize(mean=[0.4914, 0.4822, 0.4465], std=[0.2023, 0.1994, 0.2010])]
                val_trans = [transforms.ToImage(), transforms.ToDtype(torch.float32, scale=True), transforms.Normalize(mean=[0.4914, 0.4822, 0.4465], std=[0.2023, 0.1994, 0.2010])]
            case "stl10_train":
