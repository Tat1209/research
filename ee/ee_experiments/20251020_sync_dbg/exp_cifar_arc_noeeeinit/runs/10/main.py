...
optim_lr = [("sgd", 0.1), ("adam", 0.1), ("adam", 5e-3)]
...
for optim_str, max_lr in optim_lr:
    for wd in wd_l:
        ...
        runs_mgr.log_param("ee_init", ee_init:=False)
        runs_mgr.log_text(src_name, src_text)

        trainers = []
        for channels, ensembles in chs_ens_l:
            network = Network(net(num_classes=num_classes, channels=channels, ensembles=ensembles, merge_mode="both", ee_init=ee_init))
