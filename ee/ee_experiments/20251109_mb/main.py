...
from ee_tools.models.mobilenet_v2_cifar_ee import mobilenet_v2 as mbv2_cifar_ee
...
def main():
    ...
    chs_l = [base_chs // div for div in div_l]
    ...
    for optim_str, max_lr in optim_lr:
        for ndata, epochs in ndata_epochs_l:
            for wd in wd_l:
                ...
                runs_mgr.log_param("base_chs", base_chs)
                runs_mgr.log_text(src_name, src_text)

                trainers = []
                # for channels, ensembles in zip(chs_l, ens_l):
                    # network = Network(net(num_classes=num_classes, channels=channels, ensembles=ensembles, merge_mode="both"))
                for div in div_l:
                    network = Network(net(num_classes=num_classes, ee_div=div, merge_mode="both"))
