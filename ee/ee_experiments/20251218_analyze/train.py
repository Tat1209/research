...
def exp(cfg: dict):
    ...
    run.log_param("train_dataset", train_ds.state.dataset_id)
    run.log_param("val_dataset", val_ds.state.dataset_id)
    run.log_param("num_classes", num_classes := train_ds.fetch_classes())

    run.log_param("train_trans", repr(train_trans))
    run.log_param("val_trans", repr(val_trans))

    run.log_param("train_ndata", len(train_ds))
    run.log_param("val_ndata", len(val_ds))

    run.log_param("epochs", epochs)
    run.log_param("max_lr", max_lr)
    run.log_param("wd", wd)
    run.log_param("batch_size", batch_size)

    run.log_param("iters/epoch", len(train_dl))
    run.log_param("iters", len(train_dl) * epochs)
    run.log_param("processed_ndata", ndata * epochs)
    run.log_param("ndata_per_class", len(train_ds) / num_classes)
    run.log_param("div", div)
    run.log_param("ensembles", ens)
    run.log_param("scale_ch", scale_ch)
    run.log_text(src_name, src_text)
    ...
    run.log_params(hp_dict)
    run.log_text("model_repr.txt", network.repr_network())
    run.log_text("model_torchinfo.txt", network.torchinfo(dl=train_dl))
    ...
    for e in range(epochs):
        ...
        run.log_metrics(mets, step=e + 1)
        run.log_metrics(trainer.time_stats(incl_fmt=True), step=e + 1)
        run.log_metric("progress", f"{(e + 1) / epochs * 100:.1f}%", step=e + 1)
        run.sync(step=e + 1, itv=epochs/100, last_step=epochs)

        trainer.printmet(mets, e + 1, epochs, itv=epochs / 5)

        torch.save(trainer.network.state_dict(), run.fpath("state_dict.pt"))
        exp.ref_results()

        print(f"Experiment completed. Results saved to {run.run_path}")