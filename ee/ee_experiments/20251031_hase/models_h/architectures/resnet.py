...
from .blocks import BasicBlock, Bottleneck, View, CopyConcat, SplitMean, conv1x1
...
class ResNet(nn.Module):
    ...
    def get_features_and_logits(self, x: Tensor):
        """Get both features and logits for ensemble training"""
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool(x)
