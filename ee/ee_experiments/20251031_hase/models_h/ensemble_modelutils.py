...
def dkd_divergence(logit_p, logit_q, labels, alpha=1.0, beta=8.0, T=1.0, epsilon=1e-10):
    ...
    # KL Divergence の計算
    tckd_kl = F.kl_div(torch.log(p_tgt), q_tgt, reduction="batchmean") + F.kl_div(torch.log(p_ntgt), q_ntgt, reduction="batchmean")
    nckd_kl = F.kl_div(torch.log(p_nctgt), q_nctgt, reduction="batchmean")
    ...
    def train_one_epoch(self, dataloader, train=True, regularize=True, epoch=0):
        ...
        for batch in tqdm(dataloader):
            try:
                ...
                print(f"Batch type: {type(batch)}, length: {len(batch) if hasattr(batch, '__len__') else 'N/A'}")
