...
class InvertedResidual(nn.Module):
    def __init__(
        self, inp: int, oup: int, stride: int, expand_ratio: int, norm_layer: Optional[Callable[..., nn.Module]] = None, groups: int = 1
    ) -> None:
        super().__init__()
        self.stride = stride
        ...
        if expand_ratio != 1:
            # pw
            layers.append(
                Conv2dNormActivation(inp, hidden_dim, kernel_size=1, norm_layer=norm_layer, activation_layer=nn.ReLU6, groups=groups)
            )
        layers.extend(
            [
                # dw
...
class MobileNetV2(nn.Module):
    def __init__(
        ...
        self.rp_data = RepeatData(ee_div**2)
        self.chunk_merge = ChunkMerge(ee_div**2, mode=merge_mode)

        # building first layer
        input_channel = _make_divisible(input_channel * width_mult, round_nearest)
        self.last_channel = _make_divisible(last_channel * max(1.0, width_mult), round_nearest)
        features: List[nn.Module] = [
            Conv2dNormActivation(3 * ee_div**2, input_channel * ee_div, stride=2, groups=ee_div**2, norm_layer=norm_layer, activation_layer=nn.ReLU6)
        ]
        ...
        for t, c, n, s in inverted_residual_setting:
            ...
            for i in range(n):
                ...
                features.append(block(input_channel * ee_div, output_channel * ee_div, stride, expand_ratio=t, norm_layer=norm_layer, groups=ee_div**2))
                input_channel = output_channel
        # building last several layers
        features.append(
            Conv2dNormActivation(
                input_channel * ee_div, self.last_channel * ee_div, kernel_size=1, groups=ee_div**2, norm_layer=norm_layer, activation_layer=nn.ReLU6
            )
        )
        ...
        self.classifier = nn.Sequential(
            ...
            GroupedLinear(self.last_channel * ee_div, num_classes * ee_div**2, groups=ee_div**2),
        )
        ...
        for m in self.modules():
            ...
            elif isinstance(m, GroupedLinear):
