"""
A ResNet model that combines the CIFAR-adapted architecture from weiaicunzai's
implementation with the EasyEnsemble (EE) technique.

Source for base architecture:
https://github.com/weiaicunzai/pytorch-cifar100/blob/master/models/resnet.py

This model uses a CIFAR-friendly structure (3x3 initial kernel, no initial 
maxpool) and applies grouped convolutions to train multiple ensemble members 
simultaneously.
...
from ..modules.ops import RepeatData, GroupedLinearConv1d, ChunkMerge
...
class ResNet(nn.Module):
    def __init__(
        ...
        self.fc = GroupedLinearConv1d(channels * 8 * block.expansion * self.ensembles, num_classes * self.ensembles, groups=self.ensembles, bias=True)
        ...
        for m in self.modules():
            ...
            if isinstance(m, nn.Conv1d):
                chunks = m.weight.chunk(m.groups, dim=0)
                for w in chunks:
    ...
    def _make_layer(
        ...
        layers = []
        # print(f"self.inplanes={self.inplanes}, planes={planes}, self.groups={self.groups}")
        layers.append(
                block(
