...
networks = Networks(merge_stat=True)
for _ in range(ensembles):
    network = Network(net(num_classes=num_classes, nb_fils=fils))
    networks.append(network)

criterion = torch.nn.CrossEntropyLoss()
optimizer = torch.optim.SGD(networks.parameters(), lr=max_lr, momentum=0.9, weight_decay=5e-4, nesterov=True)
...
mtrainer = MergeEnsemble(networks, criterion, optimizer, scheduler_t, device)

hp_dict = {
    "params": networks.count_params(),
...
run_mgr.log_text("model_repr.txt", mtrainer.networks.repr_network())
run_mgr.log_text("model_torchinfo.txt", mtrainer.networks.torchinfo(dl=train_dl))
...
for e in range(epochs):
    ...
    run_mgr.log_metrics(mtrainer.timeinfo(), step=e + 1)
...
# run_mgr.log_torch_save(mtrainer.networks.get_sd(), "state_dict.pt")
