...
# train_trans = [transforms.ToImage(), transforms.CenterCrop(85), transforms.RandomHorizontalFlip(), transforms.RandomRotation(degrees=15, fill=(0, 0, 0)), transforms.ToDtype(torch.float32, scale=True), normalize]
# val_trans = [transforms.ToImage(), transforms.CenterCrop(85), transforms.ToDtype(torch.float32, scale=True), normalize]
train_trans = [transforms.ToImage(), transforms.RandomHorizontalFlip(), transforms.RandomRotation(degrees=15, fill=(0, 0, 0)), transforms.ToDtype(torch.float32, scale=True), normalize]
val_trans = [transforms.ToImage(), transforms.ToDtype(torch.float32, scale=True), normalize]
...
for max_lr in max_lr_l:
    ...
    # network = Network(net(weights="IMAGENET1K_V2")).tl_setup(num_classes=4, linear_layer="")
    criterion = torch.nn.CrossEntropyLoss()
    # optimizer = torch.optim.SGD(network.parameters(), lr=max_lr, momentum=0.9, weight_decay=5e-4, nesterov=True)
    optimizer = torch.optim.AdamW(network.parameters(), lr=max_lr)
